...
export function checkWarning(w) {
  ...
  const canon = squash(CANONICAL_WARNING).toLowerCase().replace(/[^a-z0-9() ]/g, "");
  const got = found.toLowerCase().replace(/[^a-z0-9() ]/g, "");
  if (!(canon === got || similarity(canon, got) >= 0.97))
...
export function runChecks(mode, application, extracted) {
  const checks = [];
  if (mode === "single") {
    checks.push(checkBrand(application.brandName, extracted.brand_name));
    checks.push(checkAbv(application.abv, extracted.alcohol_content, extracted.abv_percent));
    ...
    checks.push(checkPresence("Alcohol content", extracted.alcohol_content));
